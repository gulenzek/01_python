    ...
    for harf in text:
        if harf.isalpha():
            if harf in tekrar:
                ...
                tekrar[harf] = 1
        tekrar2 = [(k, tekrar[k]) for k in sorted(tekrar, key=tekrar.get, reverse=True)]
    print(tekrar2)

    for word, freq in tekrar2:
        if len(word) == 2 and word.endswith("n"):
            print(word, freq, "_______")

    for word, freq in tekrar2:
        if len(word) == 2 and word.startswith("m"):
            print(word, freq, "//////")

    return tekrar2


def replace_password(text):
    mapping = {}
    mapping["m"] = "t"
    mapping["f"] = "h"
    mapping["n"] = "e"
    ...
    mapping["z"] = "o"
    # mapping["a"] = "r"  # supheliyiz.

    mapping["t"] = "n"
    mapping["p"] = "b"  # be?

    text2 = text
    for eski_harf in mapping:
        yeni_harf = mapping[eski_harf]
        text2 = text2.replace(eski_harf, yeni_harf)
    return text2
